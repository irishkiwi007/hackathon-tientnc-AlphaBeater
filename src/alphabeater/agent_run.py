...
from alphabeater.backtest import BacktestResult, FactorBacktester
...
from alphabeater.models import FactorCandidate, MarketObservation
...
def _best_candidate(
    results: list[tuple[FactorCandidate, BacktestResult]],
) -> tuple[FactorCandidate, BacktestResult]:
    if not results:
        raise ValueError("the factor agent returned no candidates")
    return max(
        results,
        key=lambda item: (
            item[1].holdout.sharpe_ratio,
            item[1].holdout.excess_return,
            -abs(item[1].holdout.max_drawdown),
        ),
    )


def run_workflow(*, execute: bool, output_path: Path) -> dict[str, Any]:
    settings = Settings()
    ...
    evaluated = [
        (
            candidate,
            backtester.run(frame, candidate.expression, candidate.expected_direction),
        )
        for candidate in factors.candidates
    ]
    candidate, backtest = _best_candidate(evaluated)
    ...
    plan = strategy.build_plan(signal, candidate, quotes, now=now)
        ...
        "candidates": [
            {
                **item_candidate.model_dump(mode="json"),
                "backtest": item_backtest.model_dump(mode="json"),
                "selected": item_candidate.name == candidate.name,
            }
            for item_candidate, item_backtest in evaluated
        ],
...
def main() -> int:
    ...
    try:
        output = run_workflow(execute=args.execute, output_path=args.output)
